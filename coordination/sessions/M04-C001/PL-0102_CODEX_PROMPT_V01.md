# PL-0102 — Codex Work Order V01

Task: **PL-0102 — Orbit-coverage model with azimuth/elevation bins**
...
This prompt: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M04-C001/PL-0102_CODEX_PROMPT_V01.md
...
1. Define a deterministic orbit-coverage model using explicit azimuth/elevation sectors around the package.
2. Map accepted pose evidence from the existing ARTrackingService/PackScan coordinate convention into coverage bins; unavailable/stale pose must not create fake coverage.
3. Support configurable ring/bin definitions so packaging presets can refine requirements without changing the core model.
4. Track covered, missing, duplicate and invalid sectors with stable IDs and completion metrics.
5. Add tests for wrap-around azimuth, elevation boundaries, unavailable pose, duplicate captures and deterministic coverage totals.
