...
This prompt: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0088_CODEX_PROMPT_V01.md
Frozen criteria: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0088_CHATGPT_AUDIT_CRITERIA_V01.md
...
Local session directory model, atomic/incremental persistence and recovery tests.

## Mandatory requirements

1. Define a session storage layout for metadata, immutable source photos, per-photo records and transient state consistent with future .packscan finalization.
2. Use atomic write/rename or equivalent crash-safe patterns so interruption cannot leave a valid-looking partially written record.
3. Persist session progress incrementally after accepted captures and meaningful state changes, never requiring all data to live only in memory.
4. Keep raw accepted photo evidence immutable and isolate temporary files with explicit cleanup/recovery rules.
5. Add failure-injection tests for interrupted writes, stale temp files, duplicate IDs and reopen validation.
...
Commit the child implementation/evidence separately, publish https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0088_CODEX_LOG_V01.md, verify remote visibility, and end the child log exactly:
