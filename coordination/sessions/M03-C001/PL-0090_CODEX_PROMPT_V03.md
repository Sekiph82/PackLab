# PL-0090 — Codex Remediation Work Order V03
...
This prompt: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0090_CODEX_PROMPT_V03.md
...
1. Make launch discovery use the same authoritative ScanSessionStore reopen/recovery validator so stale temps and partial transactions are handled before resumability decisions.
2. Validate metadata, state, per-photo records and source files together before offering Resume.
3. On Resume, reconstruct and install a real active session model with sequence/history/epoch; on Discard, execute the defined safe delete/quarantine path.
4. Add restart tests for clean session, partial/crash transaction, stale temp, missing/corrupt record/source, version mismatch and discard.
