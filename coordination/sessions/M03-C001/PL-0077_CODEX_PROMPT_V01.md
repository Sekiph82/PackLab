# PL-0077 — Codex Work Order V01

Task: **PL-0077 — Add camera-error recovery for interruption, permission denial and session restart.**
...
This prompt: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0077_CODEX_PROMPT_V01.md
Frozen criteria: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0077_CHATGPT_AUDIT_CRITERIA_V01.md
...
Camera session recovery state machine, UI messaging and tests.

## Mandatory requirements

1. Handle permission denied/restricted, runtime camera errors, session interruption, interruption end and restart requests with explicit states.
2. Make recovery idempotent and prevent duplicate sessions/listeners when SwiftUI lifecycle events repeat.
3. Preserve any already-accepted immutable captures across camera-session recovery and never mark failed in-flight frames accepted.
4. Surface actionable user messages for nonrecoverable permission/capability failures and bounded retry behavior for recoverable session errors.
5. Add deterministic state-machine tests for interruption, restart success/failure and permission transitions.
...
Commit the child implementation/evidence separately, publish https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0077_CODEX_LOG_V01.md, verify remote visibility, and end the child log exactly:
