# PL-0080 — Codex Remediation Work Order V03
...
This prompt: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0080_CODEX_PROMPT_V03.md
...
1. Define and implement an explicit timestamp-domain bridge between accepted still capture time and ARFrame monotonic timestamp; do not directly compare wall-clock Date to ARKit uptime seconds.
2. Invoke pose binding from the real accepted-still pipeline, not only a standalone helper.
3. Persist alignment delta/status with the accepted capture and reject stale/ineligible tracking deterministically.
4. Add tests for domain conversion, exact tolerance boundaries, accepted-capture binding, out-of-order frames and invalid/stale pose.
