# PL-0074 — Codex Work Order V01

Task: **PL-0074 — Implement exposure metering and optional exposure lock for consistent image sets.**
...
This prompt: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0074_CODEX_PROMPT_V01.md
Frozen criteria: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0074_CHATGPT_AUDIT_CRITERIA_V01.md
...
Exposure control adapter/state UI/tests on the existing camera session.

## Mandatory requirements

1. Implement supported exposure metering/continuous-auto behavior and an optional lock path for consistent capture sets.
2. Clamp/validate exposure target bias or point-of-interest inputs against actual device capabilities and limits.
3. Record exposure mode/state used for each capture-ready state; unsupported lock must remain visibly unavailable.
4. Protect device configuration changes with the existing serialized camera configuration path and recover cleanly from configuration errors.
5. Add deterministic tests for capability gating, bounds and state transitions; do not implement white-balance logic in this child.
...
Commit the child implementation/evidence separately, publish https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0074_CODEX_LOG_V01.md, verify remote visibility, and end the child log exactly:
