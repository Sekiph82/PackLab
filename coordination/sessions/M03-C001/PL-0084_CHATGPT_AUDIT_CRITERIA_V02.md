# PL-0084 — ChatGPT Remediation Audit Criteria V02
...
10. Wire reset/relocalization through the single ARSession owner and use explicit ARSession run options only when required.
11. Connect user-requested, interruption and tracking-degradation reset triggers to the orchestration path.
12. Preserve immutable accepted captures while preventing pose continuity across localization epochs.
13. Persist reset reason/epoch diagnostics, expose relocalizing/recovered/failed state, and test idle/active/degraded/repeated resets.
