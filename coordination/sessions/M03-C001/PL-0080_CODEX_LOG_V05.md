# PL-0080 Codex Implementation Log V05
...
Implemented only the frozen PL-0080 remediation scope: `ContentView.swift` and `PackLabCaptureTests.swift`.

CaptureRuntimeViewModel now provides the live admission-controlled capture-to-ScanSessionStore path. Accepted still persistence uses the capture timestamp bridge and writes pose/motion/state transactionally; reopen reads the persisted accepted binding.
...
`testPL0080AndPL0081AcceptedStillOrchestrationPersistsPoseMotionAndReopens` covers live ViewModel capture, storeAcceptedCapture(still:), monotonic timestamp handling, reopen binding, stale/unavailable/out-of-order samples.
