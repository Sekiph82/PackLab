# PL-0093 — Codex Work Order V01

Task: **PL-0093 — Add safe deletion requiring confirmation and cleaning all associated data.**
...
This prompt: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0093_CODEX_PROMPT_V01.md
Frozen criteria: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0093_CHATGPT_AUDIT_CRITERIA_V01.md
...
Local scan/session deletion workflow, confirmation UI, file cleanup and tests.

## Mandatory requirements

1. Require an explicit confirmation step identifying the scan/session before destructive deletion.
2. Delete only files owned by the selected PackLab session/history record using validated canonical paths; prevent path traversal or deletion outside the PackLab storage root.
3. Clean associated derivatives/temp/index records consistently while never following untrusted symlinks/paths into unrelated user data.
4. Treat partial deletion as an error state with recoverable diagnostics rather than falsely reporting success.
5. Add filesystem tests for normal deletion, cancellation, missing files, partial failure and malicious/out-of-root path attempts. Stop before M04.
...
Commit the child implementation/evidence separately, publish https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0093_CODEX_LOG_V01.md, verify remote visibility, and end the child log exactly:
