...
This prompt: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0079_CODEX_PROMPT_V01.md
Frozen criteria: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0079_CHATGPT_AUDIT_CRITERIA_V01.md
...
ARTrackingService physical implementation, simulator fallback preservation, lifecycle tests.

## Mandatory requirements

1. Implement ARWorldTrackingConfiguration using non-LiDAR world tracking only; do not require sceneDepth, mesh reconstruction or Pro-only sensors.
2. Expose start/stop/state through the existing ARTrackingService seam with truthful unsupported/unavailable behavior.
3. Configure only capabilities available on the iPhone 16 Standard baseline and guard ARWorldTrackingConfiguration support before starting.
4. Keep AR session ownership/lifecycle isolated from SwiftUI and camera UI, avoiding duplicate ARSession creation.
5. Add deterministic lifecycle/capability tests where possible and preserve simulator .unavailable behavior.
...
Commit the child implementation/evidence separately, publish https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0079_CODEX_LOG_V01.md, verify remote visibility, and end the child log exactly:
