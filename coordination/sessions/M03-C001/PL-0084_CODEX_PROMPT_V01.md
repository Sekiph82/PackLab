# PL-0084 — Codex Work Order V01

Task: **PL-0084 — Implement capture-session reset/relocalization behavior.**
...
This prompt: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0084_CODEX_PROMPT_V01.md
Frozen criteria: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0084_CHATGPT_AUDIT_CRITERIA_V01.md
...
AR/camera capture session orchestration reset/relocalization path and tests.

## Mandatory requirements

1. Provide an explicit user/system-triggered reset path for AR tracking/capture coordination without deleting immutable accepted photos.
2. Reset transient AR world-tracking state using appropriate ARSession run options only when required and record a new localization segment/epoch.
3. Prevent poses from different reset epochs being silently treated as one continuous coordinate frame.
4. Define relocalizing/recovered/failed states and preserve diagnostics explaining why a reset occurred.
5. Add deterministic orchestration tests for reset during idle/active/degraded states and repeated reset requests.
...
Commit the child implementation/evidence separately, publish https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0084_CODEX_LOG_V01.md, verify remote visibility, and end the child log exactly:
