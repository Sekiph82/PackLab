# PL-0093 — Codex Remediation Work Order V03
...
This prompt: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0093_CODEX_PROMPT_V03.md
...
1. Reject non-authoritative SessionDeletionPlan instances at the destructive execution boundary; only validated session/history identities may be deleted.
2. Make the real SessionDeletionView path coordinate all authoritative history/index cleanup rather than relying on an optional unused parameter.
3. Preserve detailed partial-failure paths/results for recovery/diagnostics instead of collapsing them to an opaque partialFailure.
4. Add filesystem tests for non-authoritative rejection, success, cancellation, missing session, injected partial failure, traversal and real symlink escape.
