# PL-0076 — Codex Work Order V01

Task: **PL-0076 — Capture and persist camera metadata for every accepted still.**
...
This prompt: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0076_CODEX_PROMPT_V01.md
Frozen criteria: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0076_CHATGPT_AUDIT_CRITERIA_V01.md
...
Per-photo capture metadata model, PackScan-compatible persistence and contract tests.

## Mandatory requirements

1. Create/persist per-accepted-photo metadata covering filename/capture ID, capture timestamp, orientation, dimensions, lens identity, focal data when available, exposure, ISO, white-balance state and capture sequence.
2. Align field names/types/units with the accepted PackScan per-photo/camera contracts from M02 rather than inventing a competing schema.
3. Bind metadata atomically to the corresponding immutable source image so an accepted still cannot exist with mismatched metadata.
4. Represent unavailable hardware values explicitly and truthfully; do not synthesize EXIF/device values.
5. Add contract tests/fixtures proving accepted records validate and mismatched/missing required bindings fail closed.
...
Commit the child implementation/evidence separately, publish https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M03-C001/PL-0076_CODEX_LOG_V01.md, verify remote visibility, and end the child log exactly:
