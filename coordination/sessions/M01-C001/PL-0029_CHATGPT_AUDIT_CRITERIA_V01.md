# PL-0029 — ChatGPT Strict Child Audit Criteria V01
...
8. Choose a reproducible Python dependency-management/locking approach compatible with the pinned Python version and public/free tooling is satisfied.
9. Create the lock/config files required by that choice and a Windows bootstrap script that validates prerequisites before installing is satisfied.
10. Lock only dependencies actually authorized by M01; do not select the deferred Python OCCT binding is satisfied.
11. Make bootstrap idempotent enough to rerun safely and fail clearly on version/tool mismatch is satisfied.
12. Record exact tool/version provenance and license-sensitive package notes where relevant is satisfied.
13. Do not embed credentials, absolute owner paths or private indexes is satisfied.
...
18. Child log exists at https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M01-C001/PL-0029_CODEX_LOG_V01.md and links the full prompt/criteria URLs.
...
Any failed mandatory criterion keeps PL-0029 open. A child failure does not erase already accepted earlier child audits, but M01 remains open.
