# PL-0030 — Codex Child Work Order V01
...
Log: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M01-C001/PL-0030_CODEX_LOG_V01.md
...
1. Configure deterministic Python linting and formatting with Ruff or an equivalently justified tool already selected by the task.
2. Define a type-check strategy and baseline configuration compatible with the package layout.
3. Integrate commands into the root task runner without duplicating configuration.
4. Apply formatting only to M01-owned/new Python files unless a separate change is required and justified.
5. Add a validation that intentionally bad fixture/config would be detected or otherwise prove checks are active, not no-op.
