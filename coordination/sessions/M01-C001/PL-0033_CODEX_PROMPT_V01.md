# PL-0033 — Codex Child Work Order V01
...
Log: https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M01-C001/PL-0033_CODEX_LOG_V01.md
...
- `core/src/packlab_core/config.py`
...
1. Implement typed configuration with deterministic precedence: defaults < user config < project config < environment overrides, unless a different order is explicitly justified.
2. Keep secrets out of persisted public examples and logs.
3. Support platform-safe user config locations and explicit project config paths.
4. Validate types/unknown critical values and surface actionable errors rather than silently coercing dangerous values.
5. Add tests for precedence, missing files, malformed values and environment overrides.
