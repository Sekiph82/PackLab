# PL-0030 — ChatGPT Strict Child Audit Criteria V01
...
8. Configure deterministic Python linting and formatting with Ruff or an equivalently justified tool already selected by the task is satisfied.
9. Define a type-check strategy and baseline configuration compatible with the package layout is satisfied.
10. Integrate commands into the root task runner without duplicating configuration is satisfied.
11. Apply formatting only to M01-owned/new Python files unless a separate change is required and justified is satisfied.
12. Add a validation that intentionally bad fixture/config would be detected or otherwise prove checks are active, not no-op is satisfied.
...
17. Child log exists at https://github.com/Sekiph82/PackLab/blob/main/coordination/sessions/M01-C001/PL-0030_CODEX_LOG_V01.md and links the full prompt/criteria URLs.
...
Any failed mandatory criterion keeps PL-0030 open. A child failure does not erase already accepted earlier child audits, but M01 remains open.
